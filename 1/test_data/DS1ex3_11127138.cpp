...
class Maze{
	public:
		...
		bool findShortestPath( int currenty, int currentx ) ;
		void show( bool success ) ;
		void showShortestPath( bool success ) ;
		void clean() ;
	...
	private:
        ...
        vector<vector<char> > patternVBack;
...
bool Maze::findShortestPath( int currenty, int currentx ) {
    //countS = countS + 1 ;

	temproute.push_back( make_pair(currenty,currentx) ) ; // store temp pass

	patternV[ currenty ][ currentx ] = 'V' ;
    patternVBack[ currenty ][ currentx ] = 'V' ;
    //showShortestPath( false ) ;
    /*
    for(int i = 0 ; i < temproute.size() ; i++){
        cout<<"("<<temproute[i].second+1<<","<<temproute[i].first+1<<") " ;
    }*/
	...
	int trydirection = 0 ;
	if ( pattern[ currenty ][ currentx ] == 'G' ){
        patternVBack[ currenty ][ currentx ] = 'G' ;
        if ( temproute.size() < shortest ){
            shortest = temproute.size() ;
            //countS = 0 ;
	...
	// decide move to which place, according to the counterclockwise principle
	while( pattern[ currenty ][ currentx ] != 'G' && trydirection != 4 && temproute.size() < shortest-1 ){
		//int tempy, tempx ;
		if( direction == Right ) {
			if ( (currentx+1 < x) && (patternVBack[ currenty ][ currentx+1 ] != 'O') && (patternVBack[ currenty ][ currentx+1 ] != 'V') )
				findShortestPath( currenty, currentx + 1 ) ;
			direction = Down ;
		}
		else if( direction == Down ) {
			if ( (currenty+1 < y) && (patternVBack[ currenty+1 ][ currentx ] != 'O') && (patternVBack[ currenty+1 ][ currentx ] != 'V') )
				findShortestPath( currenty + 1, currentx ) ;
			direction = Left ;
		}
		else if( direction == Left ) {
			if ( (currentx-1 >= 0) && (patternVBack[ currenty ][ currentx-1 ] != 'O') && (patternVBack[ currenty ][ currentx-1 ] != 'V') )
				findShortestPath( currenty, currentx - 1 ) ;
			direction = Up ;
		}
		else if( direction == Up ) {
			if ( (currenty-1 >= 0) && (patternVBack[ currenty-1 ][ currentx ] != 'O') && (patternVBack[ currenty-1 ][ currentx ] != 'V') )
				findShortestPath( currenty - 1, currentx ) ;
			direction = Right ;
		}
		trydirection++ ;
	...
	if ( patternVBack[ currenty ][ currentx ] == 'G' ){
        patternVBack[ currenty ][ currentx ] = 'G' ;
        if ( temproute.size() < shortest ){
            shortest = temproute.size() ;
            //countS = 0 ;
    ...
    patternVBack[ temproute[temproute.size()-1].first ][ temproute[temproute.size()-1].second ] = 'E' ; // 回溯temprouteBack
	temproute.pop_back() ; // pop out illegal route
...
void Maze::showShortestPath( bool success ) {
	for ( int i = 0 ; i < y ; i++ ){
		for ( int j = 0 ; j < x ; j++ ) {
			if ( pattern[i][j] == 'G' )
				cout << pattern[i][j] ;
...
void Maze::clean() {
    ...
    vector<vector<char> >().swap(patternVBack);
...
int main() {
	...
	while(true) {
		...
		if ( command.compare( "0" ) == 0 )
			...
			std::cin>> fileName;
			if ( aMaze.load( fileName, mode ) ) {
				bool success = aMaze.findShortestPath( 0, 0 ) ;
				aMaze.showShortestPath( success ) ;
